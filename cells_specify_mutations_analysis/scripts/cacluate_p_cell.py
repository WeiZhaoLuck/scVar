...
def each_group(labels1,labels2):
    ...
    if(len(resultsP)>0):
        ...
        path_out=os.path.join("results3",name)
...
if __name__ == '__main__':
    all_ref=pd.read_csv("all_table_ref_mutation_cell_type.csv",sep=",",index_col=0)
    all_alt=pd.read_csv("all_table_alt_mutation_cell_type.csv",sep=",",index_col=0)
