...
for i in range(len(merge_result)):
    mutation=SNV_all[i]
    mutations_each_gene_data_sum_alt=merge_result.iloc[i,1]
    if isinstance(merge_result.iloc[i,1],list):
        ...
        for column_name, column_data in sum_alt.items():
            out_data_alt.loc[mutation,column_name] = column_data
    if isinstance(merge_result.iloc[i,2],list):
        mutations_each_gene_data_sum_ref=merge_result.iloc[i,2]
        ...
        for column_name, column_data in sum_ref.items():
            out_data_ref.loc[mutation,column_name] = column_data
out_data_alt.to_csv("BT1292_1/all_table_alt_mutation_cell_type.csv",sep=",")
out_data_ref.to_csv("BT1292_1/all_table_ref_mutation_cell_type.csv",sep=",")

# mutation_Gene=pd.read_csv("BT1297_1/mutaion.info",sep="\t",header=0,names=['mutation','gene'],index_col=False)
# barcode_cluster_info=pd.read_csv("BT1297_1/barcodes_cell_type.tsv",sep="\t",header=None,names=['barcode','cluster'],index_col=False)
...
#         sum_alt=Counter(barcode_cluster_info[barcode_cluster_info['barcode'].isin(mutations_each_gene_data_sum_alt)]['cluster'])
#         for column_name, column_data in sum_alt.items():
#             out_data_alt.iloc[i,column_name] = column_data
#         mutations_each_gene_data_sum_ref=list(set(mutations_each_gene_data.explode('barcode_y')['barcode_y'].tolist()))
#         sum_ref=Counter(barcode_cluster_info[barcode_cluster_info['barcode'].isin(mutations_each_gene_data_sum_ref)]['cluster'])
#         for column_name, column_data in sum_ref.items():
#             out_data_ref.iloc[i,column_name] = column_data
# out_data_alt.to_csv("BT1297_1/all_table_alt_gene_cell_type.csv",sep=",")
# out_data_ref.to_csv("BT1297_1/all_table_ref_gene_cell_type.csv",sep=",")
