...
def extract_bam(barcodes_path, chr, out_path):
    ...
    for cell_type in ALL_CELL_TYPES:
        outfile="{}/{}_{}.bam".format(out_path, chr,cell_type)
        outfile_wb = pysam.AlignmentFile(outfile, "wb",template=bam_all)
    ...
    with pysam.AlignmentFile(tmpfilename, "wb", header=bam_all.header) as outf:
        for item in bam_all:
            if item.has_tag('CB'):
                ...
                if a in barcodes:
                    outf.write(item)
                    cell_type = barcode_map[a]
                    DICT_files[cell_type].write(item)
